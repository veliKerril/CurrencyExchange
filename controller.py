...
from model import MyException1, MyException2
...
class HTTPRequestHandler(server.BaseHTTPRequestHandler):
    ...
    def do_post_add_new_currency(self):
        ...
        # Если выбрасывается KeyError, то значит нет какого-то поля
        except KeyError:
            self.send_response(400)
            self.send_header('content-type', 'text/html')
            self.end_headers()
    ...
    def do_post_exchange_rate(self):
        ...
        # Эта ошибка вызывается при отсутсвии нужного поля формы
        except KeyError:
            self.send_response(400)
            self.send_header('content-type', 'text/html')
            self.end_headers()
